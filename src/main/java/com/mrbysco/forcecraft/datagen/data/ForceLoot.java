...
public class ForceLoot extends LootTableProvider {
	...
	protected List<Pair<Supplier<Consumer<BiConsumer<ResourceLocation, Builder>>>, LootContextParamSet>> getTables() {
		return ImmutableList.of(
				Pair.of(ForceLoot.ForceBlocks::new, LootContextParamSets.BLOCK),
				Pair.of(SpoilsBagLootTables::new, LootContextParamSets.GIFT)
		);
	}

	private static class ForceBlocks extends BlockLoot {
		private static final LootItemCondition.Builder HAS_SILK_TOUCH = MatchTool.toolMatches(net.minecraft.advancements.critereon.ItemPredicate.Builder.item().hasEnchantment(new EnchantmentPredicate(Enchantments.SILK_TOUCH, Ints.atLeast(1))));
		private static final LootItemCondition.Builder HAS_SHEARS = MatchTool.toolMatches(net.minecraft.advancements.critereon.ItemPredicate.Builder.item().of(new ItemLike[]{
				Items.SHEARS}));
			...
			add(FORCE_LEAVES.get(), (leaves) -> {
				...
				pool.withPool(
						LootPool.lootPool()
								...
								.add(
										((LootPoolSingletonContainer.Builder<?>) applyExplosionCondition(leaves, LootItem.lootTableItem(FORCE_NUGGET.get())))
												.when(
