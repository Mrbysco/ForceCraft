	...
	public void biomeLoadingEvent(BiomeLoadingEvent event) {
		...
		if(category != BiomeCategory.THEEND && category != BiomeCategory.NETHER) {
			if(ConfigHandler.COMMON.generateForceOre.get()) {
				builder.getFeatures(GenerationStep.Decoration.UNDERGROUND_ORES).add(() -> ForcePlacements.ORE_FORCE);
				builder.getFeatures(GenerationStep.Decoration.UNDERGROUND_ORES).add(() -> ForcePlacements.ORE_FORCE_BURIED);
			}
			if(ConfigHandler.COMMON.generateForceTree.get() && category == BiomeCategory.FOREST) {
				builder.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, ForcePlacements.FORCE_TREE_CHECKED);
			}
		}
	}
}
